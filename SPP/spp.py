...
time_start = [2021, 3, 1, 0, 0, 0]
time_end = [2021, 3, 2, 0, 0, 0]
dt = 30
week_start, tow_start, dow_start = date2tow(time_start)
week_end, tow_end, dow_end = date2tow(time_end)

nav,inav = readrnxnav("WROC00POL_R_20210600000_01D_GN.rnx")
obs, iobs, obsRECV = readrnxobs('WROC00POL_R_20210600000_01D_30S_MO.rnx', time_start, time_end, 'G')
XYZWro = obsRECV.copy()


def el_az(sat):
    fi, lamb, h = hirvonen(obsRECV[0], obsRECV[1], obsRECV[2])
    ...
    xyz = np.array([obsRECV[0], obsRECV[1], obsRECV[2]])
...
def dops(A):
    ...
    fi, lam, h = hirvonen(obsRECV[0], obsRECV[1], obsRECV[2])
...
def matrixA(satellite, dist):
    matA = [(-(satellite[0] - obsRECV[0]) / dist)[0], (-(satellite[1] - obsRECV[1]) / dist)[0],
            (-(satellite[2] - obsRECV[2]) / dist)[0], 1]
    return matA
...
def iono(time, el, az):
    ...
    fir, lamr, hr = hirvonen(obsRECV[0], obsRECV[1], obsRECV[2])
    tow = time
    els = el / 180
    # kat geocentryczny
...
def epoch_param(nrSatellite, time):
    frame = nav[np.where(np.any(inav == nrSatellite, axis=1))[0]]
...
def geom_dist(XYZ):
    ro = math.sqrt((XYZ[0] - obsRECV[0]) ** 2 + (XYZ[1] - obsRECV[1]) ** 2 + (XYZ[2] - obsRECV[2]) ** 2)
    return ro


def pseudo_dist(satellite, dist, corr_iono, corr_topo):
    if corr_iono == 0:
        pseudoDist = dist - c * satellite[3] + c * obsRECV[3] + corr_topo
    if corr_topo == 0:
        pseudoDist = dist - c * satellite[3] + c * obsRECV[3] + corr_iono
    else:
        pseudoDist = dist - c * satellite[3] + c * obsRECV[3] + corr_topo + corr_iono
...
def differences_dops(t_corr, i_corr):
    ...
    dop = np.zeros([0, 5])
    i = 0
    for time in range(tow_start, tow_end, dt):
        recv = np.zeros([0, 4])
        dist = np.zeros([0, 1])
        T = 0.072
        idx = iobs[:, 2] == time
        ...
        for j in range(4):
            ...
            obsRECV[1] += matX[1, 0]
            obsRECV[2] += matX[2, 0]
            obsRECV[3] += matX[3, 0] / c
            recv = np.vstack([recv, obsRECV])
            last = recv[-1]
